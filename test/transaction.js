...
it('serializeTx() and desrializeTx() should be deterministic', function () {
  ...
  var txobj_serialized = litecoinzjs.transaction.serializeTx(txobj)
  var txobj_deserialized = litecoinzjs.transaction.deserializeTx(txobj_serialized)
...
it('signTx() should be deterministic', function () {
  ...
  const SIGHASH_ALL = 1
  var signedobj = litecoinzjs.transaction.signTx(txobj, 0, '2c3a48576fe6e8a466e78cd2957c9dc62128135540bbea0685d7c4a23ea35a6c', compressPubKey, SIGHASH_ALL)
  var signed_serialized = litecoinzjs.transaction.serializeTx(signedobj)
...
it('multiSign() for x-of-5 should be deterministic', function () {
  ...
  // var addresses = pubKeys.map((x) => litecoinzjs.address.pubKeyToAddr(x, '2098'))
  var redeemScript = litecoinzjs.address.mkMultiSigRedeemScript(pubKeys, 2, 5)
  // var multiSigAddress = litecoinzjs.address.multiSigRSToAddress(redeemScript, '2092')
...
it('multiSign() and applyMultiSignatures() and should be deterministic', function () {
  ...
  var priv1 = litecoinzjs.address.WIFToPrivKey(privKeysWIF[0])
  var priv2 = litecoinzjs.address.WIFToPrivKey(privKeysWIF[1])
  var priv3 = litecoinzjs.address.WIFToPrivKey(privKeysWIF[2])
  ...
  var sig1 = litecoinzjs.transaction.multiSign(txobj, 0, priv3, redeemScript)
  var sig2 = litecoinzjs.transaction.multiSign(txobj, 0, priv2, redeemScript)
  ...
  expect(litecoinzjs.transaction.deserializeTx(serializedTx)).to.deep.equal(deserializedTx)
})
